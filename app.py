...
if use_flange:
    ...
    st.sidebar.caption(
        "Flange assumed along full edge length.\n"
        "Flange thickness assumed same as base plate."
    )
else:
    flange_h = 0.0
...
if use_beads:
    bead_d = st.sidebar.number_input("Bead Depth (mm)", 2.0, 20.0, 8.0)
    bead_w = st.sidebar.number_input("Bead Width (mm)", 5.0, 40.0, 20.0)
    bead_n = st.sidebar.number_input("Number of Beads", 1, 10, 3)


# ------------------------------------------------------------
# COMMON LOW-FIDELITY PHYSICS (PLATE-CONSISTENT)
# ------------------------------------------------------------
t = thk_mm / 1000.0
L = L_mm / 1000.0
b = b_mm / 1000.0
...
# Base bending inertia (plate strip)
I = (b * t**3) / 12.0

# Plate correction factor (accounts for 2D load spreading)
plate_factor = 1.6   # conservative, early-stage safe

# Low-fidelity stiffness (plate-consistent)
LF_k = plate_factor * (48.0 * E * I) / (L**3)

# ------------------------------------------------------------
# Plate load distribution correction (area vs line load)
# ------------------------------------------------------------
load_spread_factor = (b / L) ** 0.5     # non-dimensional
load_spread_factor = np.clip(load_spread_factor, 0.6, 1.0)

LF_k *= load_spread_factor
...
delta_k = np.clip(delta_k, -5.0 * LF_k, 5.0 * LF_k)

# AI-corrected stiffness (NO geometry yet)
...
bc_factor = 1.0
if use_flange:
    bc_factor = 1.25   # partial edge fixity due to flange
...
bead_factor = 1.0
if use_beads:
    bead_factor = 1.0 + 0.02 * bead_n + 0.015 * bead_d
    bead_factor = np.clip(bead_factor, 1.1, 1.7)

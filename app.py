...
if use_beads:
    ...
    bead_efficiency = 0.7
    bead_factor = 1.0 + bead_efficiency * orient_factor * 0.10 * depth_ratio * bead_count
    bead_factor = np.clip(bead_factor, 1.05, 2.5)
# combine geometry effects in a moderated way
geom_factor = np.sqrt(bc_factor * flange_factor * bead_factor)

HF_k = HF_k_raw * geom_factor

# keep physics bounds
HF_k = np.clip(HF_k, 0.7 * LF_k, 4.0 * LF_k)
